...
- `lavandula-dashboard` (gunicorn ×2) · `lavandula-orchestrator` (`run_orchestrator`) · `metric-review` (`review_server.py` @ `locard/operations/metric-review/`) · nginx · tailscale  *(local postgres REMOVED 2026-06-19)*
...
| `nonprofits/tools/enrich_990` (+ test) | self-labeled DEPRECATED; superseded by `990-index`+`990-parse` (its docstring delegates to them); 0 `990-enrich` jobs ever run | **COMMAND REMOVED 2026-06-19** (file + test + orchestrator registry runner). The inert `"990-enrich"` *phase enum* is KEPT (in `models.py` choices + `_990_PHASES` concurrency group + 2 tests) — fully removing it needs a migration for no gain |
...
- **`locard/spikes/0064/` = 5.0 GB — ⚠️ DELETE ONLY THE `*_img/` SUBDIRS, NOT THE WHOLE TREE.** The 5 GB is duplicated, regenerable PDFs + page images under `eval_set/vision/*_img/` (same source PDFs copied into ~6 review-image dirs; gitignored). Safe to delete — regenerates from S3. **🔴 DO NOT delete `eval_set/vision/*-review-data.json`** (`review-data.json`, `story-review-data.json`, `slot-review-data.json`, `new-review-data.json`, ~5 MB) — those are the **live `metric-review` service's data source** (`review_server.py` serves them), NOT regenerable junk. `rm -rf locard/spikes/0064` would take down the running service. **✅ All 4 are now git-tracked** (the smoke-test found `slot-review-data.json` + `new-review-data.json` untracked; committed 2026-06-19). `slot-review-data.json` has NO regen path, so git is its only backstop — keep it tracked. Precise instruction: delete the `*_img/` subdirs ONLY — `eval_set/vision/` also holds non-image, non-junk content (`bakeoff/` ~96M, `original_review.html`) that the image gitignore does NOT cover.
...
- **`comp-metric-regression/` = ~55 one-off research scripts.** Keepers: `review_server.py` (✅ **RELOCATED 2026-06-19** to `locard/operations/metric-review/` — was the live `metric-review.service` misplaced in the scratch dir), `regroup.py` (validated geometry method), `frozen/composed-baseline-2026-06-14/` (deliberate baseline), `fixtures/`. The other ~50 (`build_*`, `review_*`, one-off checks) → archive.
- ~~Plus `.builders/` worktree duplicates and nested venvs~~ — **removed 2026-06-19** (`.builders/0069` + `reports/venv` + `nonprofits/venv`; root venv kept — it's the active env).
