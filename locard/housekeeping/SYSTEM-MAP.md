...
- **`locard/spikes/0064/` = 5.0 GB — ⚠️ DELETE ONLY THE `*_img/` SUBDIRS, NOT THE WHOLE TREE.** The 5 GB is duplicated, regenerable PDFs + page images under `eval_set/vision/*_img/` (same source PDFs copied into ~6 review-image dirs; gitignored). Safe to delete — regenerates from S3. **🔴 DO NOT delete `eval_set/vision/*-review-data.json`** (`review-data.json`, `story-review-data.json`, `slot-review-data.json`, `new-review-data.json`, ~5 MB) — those are the **live `metric-review` service's data source** (`review_server.py` serves them), NOT regenerable junk. `rm -rf locard/spikes/0064` would take down the running service. **⚠️ Worse: of those 4, only `review-data.json` + `story-review-data.json` are git-tracked — `slot-review-data.json` and `new-review-data.json` are NOT in git, and `slot-review-data.json` has NO regen path. Back them up / commit them BEFORE any cleanup; losing `slot-review-data.json` is unrecoverable.** Precise instruction: delete the `*_img/` subdirs ONLY — `eval_set/vision/` also holds non-image, non-junk content (`bakeoff/` ~96M, `original_review.html`) that the image gitignore does NOT cover.
